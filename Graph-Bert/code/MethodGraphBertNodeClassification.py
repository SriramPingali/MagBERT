    ...
    def train_model(self, max_epoch):
        ...
        for k in range(k_fold):
            ...
            for epoch in range(max_epoch):
                ...
                output = self.forward(self.data['raw_embeddings'], self.data['wl_embedding'], self.data['int_embeddings'], self.data['hop_embeddings'], self.data['idx_train'][k])

                loss_train = F.cross_entropy(output, self.data['y'][self.data['idx_train'][k]])
                accuracy.data = {'true_y': self.data['y'][self.data['idx_train'][k]], 'pred_y': output.max(1)[1]}
                acc_train, f1_train = accuracy.evaluate()

                loss_train.backward()
                optimizer.step()

                self.eval()
                output = self.forward(self.data['raw_embeddings'], self.data['wl_embedding'], self.data['int_embeddings'], self.data['hop_embeddings'], self.data['idx_val'][k])

                loss_val = F.cross_entropy(output, self.data['y'][self.data['idx_val'][k]])
                accuracy.data = {'true_y': self.data['y'][self.data['idx_val'][k]],
                                 'pred_y': output.max(1)[1]}
                acc_val, f1_val = accuracy.evaluate()
                ...
                accuracy.data = {'true_y': self.data['y'][self.data['idx_test']],
                                 'pred_y': output.max(1)[1]}
                acc_test, f1_test = accuracy.evaluate()
